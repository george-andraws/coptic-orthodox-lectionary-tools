...
- Publish outcome: George explicitly confirmed and Hermes ran `npm publish --access public` from `packages/lectionary-data`; npm published `@andraws/lectionary-data@1.1.1` to the public registry.
- Registry verification: `npm view @andraws/lectionary-data@1.1.1 version dist.tarball dist.shasum dist.integrity time --json` returned version `1.1.1`, tarball `https://registry.npmjs.org/@andraws/lectionary-data/-/lectionary-data-1.1.1.tgz`, shasum `61414333eaa47f7d468cd1b44eda18161a19edd3`, and integrity `sha512-HiPVNPKr5ibvmnVlQZXvMjCdXsh0dIajyQraIyyQqmdR/FMWBDQeSUP7PvBuqhB/uda8kihJ5YSjPcoY6Wfiyw==`.

### Step 18 - 2026-06-19 study coverage reconnaissance and join
- Producer: `openai-codex/gpt-5.5`.
- Scope: data preparation only. No Bible-study note or vault body was edited. The final lectionary dataset was treated as valid input.
- Artifacts generated: `out/study_lectionary_coverage.csv`, `out/study_coverage_rollup.csv`, `out/lectionary_gap_no_study.csv`, `out/why_flags.csv`, `out/why_source_map.md`, `audit_artifacts/study_coverage_recon_report.md`, and `audit_artifacts/study_coverage_independent_audit.md`.
- Inputs used: `out/design/reading_identity.csv`, `out/design/reverse_lectionary_index.jsonl`, `out/data/reverse_lookup_crosswalk.csv`, `out/data/bible_chapter_lectionary_occurrences.csv`, `out/design/synaxarium_reading_bridge.csv`, `out/design/foundational_reading_collections_69.csv`, `out/design/source_registry.csv`, `lectionary_spec.md`, `coptic-lectionary-and-synaxarium.md`, and active Bible-study frontmatter from the local Obsidian vault.
- Build result: 843 active study notes parsed, 6541 coverage rows, 843 rollup rows, 3016 no-study gap rows after duplicate cleanup, and 8207 why-flag rows. The join extended no crosswalks and changed no lectionary source data.
- Source scouting: direct probes to ACCOT Youssef, UKMID Katameros Days, UKMID Katameros Sundays, CCDL Coptic Encyclopedia, and St. Bishoy Katameros returned HTTP 200. Live `web_search` was unavailable because the web backend was not configured with usable Firecrawl or portal credits.
- Independent audit: `audit_artifacts/study_coverage_independent_audit.md` reported no blockers. It first found 7 exact duplicate gap rows; those were removed, leaving 3016 no-study gap rows and zero exact duplicates.
- Verification: custom verifier checked required CSV columns, expected row counts, zero exact duplicates, identity-key joins against `reading_identity.csv` and `reverse_lectionary_index.jsonl`, rollup consistency, prose content rules for the authored report/source-map/audit files, and git diff scope. Result: passed.

### Step 19 - 2026-06-25 Pascha Wednesday reverse index correction and handoff refresh
- Producer: `openai-codex/gpt-5.5`.
- Repo-access boundary: Hermes remained inside `coptic-lectionary-research` only. No work was done in `coptic-corpus`.
- Scope: Pascha Wednesday reverse-index correction only. Coptic Reader Wednesday Day fixture rows were treated as current authority. B/C adjudications were routed to George and Fr. Boulos without applying them.
- Phase 0 and Phase 1 commit: `1f04c1a` added `audit_artifacts/pascha_wednesday_phase0_classification.md`, Grok audit artifacts, and the bounded builder/verifier changes that collapse exact duplicate Pascha Wednesday attestations while preserving source disclosure. The generated reverse index moved from `11923` rows to `11905` rows. Category A exact duplicate identities collapsed to one card each. Category B and C row keys and common rows remained unchanged.
- Independent audits: `audit_artifacts/pascha_wednesday_phase0_grok_audit.md` returned PASS WITH FIXES; `audit_artifacts/pascha_wednesday_phase0_grok_audit_pass2.md` returned PASS after reclassifying `Memoirs of Job` and clarifying Job/Wisdom caveats. `audit_artifacts/pascha_wednesday_phase1_grok_audit_pass2.md` returned PASS after the verifier and regenerated summary were aligned.
- Phase 2 commit: `a854e15` appended only the Pascha Wednesday B/C adjudication section to `audit_artifacts/open_questions_for_george.md`. It records candidate removed/historical readings and Psalm/range/named-reading artifacts as proposed-not-applied review items.
- Phase 3 commit: `8721f08` refreshed `out/handoff/reverse_lectionary_index.jsonl`, regenerated `packages/lectionary-data/`, and added `audit_artifacts/pascha_wednesday_phase3_verification.md`.
- Phase 3 verification counts: `Psalm 23` query count stayed `39` before and after; Pascha Wednesday query count changed from `65` to `47`; only the `Wednesday` occasion count changed, from `36` to `18`; Category B stayed `14` rows and Category C stayed `25` rows with zero key additions, zero key removals, and zero changed common rows.
- Deliverable hashes recorded in `audit_artifacts/pascha_wednesday_phase3_verification.md`: design and handoff reverse index share SHA-256 `ba6dcec9cdfa7a9497f2f5abd2dc2f014fdc18a8d07f2ea9e02c1ae59dc5d396`; package reverse index SHA-256 is `3e3fa60fb1255153dae5acc1f13adea1f8b98b1ed6f8a08b76ab134a2dca6493` because package projection annotates inactive provenance rows.
- Package rebuild result: `node scripts/build_npm_package.mjs` produced `@andraws/lectionary-data@1.1.6`, `source_repo_commit=a854e1585fefef5d96ce3e96202e43da5ed4e5fe`, `occasion_index_rows=11905`, `active_rows=11390`, and `removed_rows_included=515`.
- Verification: `PYTHONDONTWRITEBYTECODE=1 python3 verify_design_deliverables.py` returned `design deliverables verified`; `PYTHONDONTWRITEBYTECODE=1 python3 scripts/verify_package_integrity.py --package-dir /Users/georgeandraws/workspace/coptic-lectionary-research/packages/lectionary-data` returned `status: pass`, `reverse_index_rows=11905`, `active_reverse_index_rows=11390`, and `reverse_index_duplicate_keys=0`; `git diff --check` passed.
